/**
 * Airbnb-style data table component
 */
import React from 'react';
import type { TableProps } from '../../../types';

/**
 * Component for displaying tabular data with Airbnb-inspired design
 * @param props - Component props
 * @returns React component
 */
const AirbnbDataTable: React.FC<TableProps> = ({ title, headers, data }) => {
  // Helper function to determine if a value is positive or negative
  const getValueClass = (value: string | number): string => {
    if (typeof value === 'number') {
      return value >= 0 ? 'text-green-600' : 'text-rose-600';
    }
    
    // Try to parse string as number
    const numValue = parseFloat(String(value));
    if (!isNaN(numValue)) {
      return numValue >= 0 ? 'text-green-600' : 'text-rose-600';
    }
...
export default AirbnbDataTable;
