...
const api = {
  ...
  getCitiesAndCountries: async (): Promise<City[]> => {
    try {
      const response = await axios.get<City[]>(`${API_URL}/cities_and_countries`);
  ...
  getCountry: async (city: string): Promise<CountryResponse> => {
    try {
      ...
      const response = await axios.get<CountryResponse>(`${API_URL}/country?city=${formattedCity}`);
      return response.data;
    } catch (error) {
  ...
  getHappinessQol: async (city: string): Promise<HappinessQolItem[]> => {
    try {
      const formattedCity = encodeURIComponent(city.toLowerCase().trim());
      const response = await axios.get<HappinessQolItem[]>(`${API_URL}/happiness_qol?city=${formattedCity}`);
      return response.data;
    } catch (error) {
  ...
  getCostOfLiving: async (city: string): Promise<CostOfLivingItem[]> => {
    try {
      ...
      const formattedCity = encodeURIComponent(city.toLowerCase().trim());
      const response = await axios.get<CostOfLivingItem[]>(`${API_URL}/cost_of_living?city=${formattedCity}`);
      return response.data;
    } catch (error) {
